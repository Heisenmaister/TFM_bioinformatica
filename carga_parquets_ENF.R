...
consulta_api <- function(filtros) {
  ...
  # Ruta final fija
  parquet_dest_dir <- "F:/Bioinformatica/TFM/parquets_AF"
...
filtros_effects_samples <- list(
  ...
  samples_variants = list(
    filtro = empty_named_list()   # fuerza a {}
  ),
  variants = list(
    filtro = empty_named_list()   # fuerza a {}
  )
...
parquet_paths <- consulta_api(filtros_effects_samples)

if (!is.null(parquet_paths[["parquet_largo.parquet"]])) {
  ds_largo_AF <- open_dataset(parquet_paths[["parquet_largo.parquet"]])
  cat("Dataset largo cargado correctamente.\n")
}

if (!is.null(parquet_paths[["parquet_corto.parquet"]])) {
  ds_corto_AF <- open_dataset(parquet_paths[["parquet_corto.parquet"]])
  cat("Dataset corto cargado correctamente.\n")
}

if (exists("ds_largo_AF")) {
  cat("\nPrimeras filas del dataset largo:\n")
  print(head(collect(ds_largo_AF)))
}

if (exists("ds_corto_AF")) {
  cat("\nPrimeras filas del dataset corto:\n")
  print(head(collect(ds_corto_AF)))
}
